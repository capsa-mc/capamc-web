---
title: Ways to give
intro: CAPA-MC is a volunteer-run 501(c)(3) nonprofit. Your membership and gifts
  fund scholarships, student clubs, and advocacy for Chinese American families.
  We accept cash, check, or PayPal.
methods:
  - title: Become a member
    body: Voting membership is a $30 fee for two years, payable by PayPal or credit
      card. Membership powers our advocacy and student programs.
    cta:
      label: Join CAPA-MC TODAY!
      href: https://forms.gle/fAfKGiyVfht2QPcSA
    accent: seal
  - title: Donate by Zelle (preferable without fees) or PayPal
    body: Give securely online through Zelle or PayPal.Me, Payable to
      finance@capamc.org, or capamcfinance@gmail.com.
    cta:
      label: Donate via Zelle (Preferable without fees) or PayPal (link)
      href: https://www.paypal.com/paypalme/capamc
    accent: jade
  - title: Donate by check
    body: "Make checks payable to CAPA-MC and mail to: 12774 Wisteria Drive, #115,
      Germantown, MD 20875."
    accent: gold
---
## Volunteer with us

Beyond giving, the most valuable support is your time. CAPA-MC clubs and events are
run entirely by parent and student volunteers. Reach out to get involved in
advocacy, student clubs, scholarships, and community service.